...
const SpectractBackground: React.FC<{ children: React.ReactNode , playCount : number}> = ({
  children,
  playCount,
}) => {
;
  return (
    <div
      className="relative w-[100vw] min-h-screen overflow-hidden bg-gradient-to-br 
    ...
    >
      ...
      <div
        className="absolute rotate-x-60 bottom-8 sm:-bottom-10 left-0 w-full h-[50vw] sm:h-[50%] z-[50] overflow-hidden"
      >
        <svg
      ...
      <div className="w-[90vw] rounded-full sm:h-[85%] h-[80vh] md:scale-100 max-w-[500px] bottom-[8rem] sm:bottom-24 absolute  transform -translate-x-1/2 left-1/2 z-[900]  overflow-hidden mix-blend-lighten ">
        <motion.video
          ...
          initial={playCount > 1 ? {y : 20, opacity: 1} : {y : 800 ,opacity: 0}}
      ...
      <motion.div
        ...
        initial={playCount > 1 ? {scale: 1, opacity: 1, rotateX: 0} : { scale: 0.2, opacity: 0, rotateX: 50 }}
      ...
      >
        <motion.div
          initial={playCount > 1 ? {scale: 1, opacity: 1, rotateY: 0, rotateX: 0}:{ scale: 0.2, opacity: 0.5, rotateY: -15, rotateX: 10 }}
